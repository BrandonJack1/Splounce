...
public class GameOverAdCode : MonoBehaviour, IUnityAdsListener
{
  ...
  {
    if (ADS.adCounter == 2 && PlayerPrefs.GetString("Show Ads") != "No")
    {
      if (!Advertisement.IsReady(this.interstitalAd))
        return;
      Time.timeScale = 0.0f;
